...
image::05-whitebox-level-1.png["Whitebox-Gesamtsystem", title="Whitebox Gesamtsystem"]
...
| Verträge mit Privatkunden:
  ...
  eine Schnittstelle dieser Komponente gelesen - die eigentliche Speicherung erfolgt in der <<PK-GK-Datenbank>>.
|

| <<Configurator Shell>>| Fassade für unterschiedliche Konfiguratoren
| Die Konfiguratoren werden aus Meta-Beschreibungen generiert und in die unterschiedlichen
  Sales-Frontends integriert. Enthält auch die (regelgestützten) Expertensysteme und
  die ->CTI Anbindung. Hauptsächlich Java, relevante Teile in Prolog, Python, PHP und Flash(tm).
...
| TODO
a| vergibt Gutscheine an bestimmte Kundensegmente (z.B. Treueboni, Ferien-
...
| Order & Fulfillment
a| * Erzeugung verbindlicher Aufträge (_order_) an Lieferanten und Partner, nachdem
seitens Kunden verbindliche Bestellungen eingeganten sind.
...
| Partner Management
| Verwaltung von Partnern (Adressen, Kontaktdaten) sowie Zuordnung zu bestimmten Produktkategorien
in VENOM.
|
...
| Ursprünglich von Blau&Partner, danach von ATLAS Software entwickeltes System für
  ...
  Die Datenhaltung der hierüber verkauften Produkte erfolgt
  in einer C++ Objektdatenbank (http://www.eyedb.org/[EyeDB^]).


| Sales Backend Private + Corporate
| Abwicklung von Verkäufen für Privat- und Firmenkunden,
  ebenfalls -><<NGO,NGOs>>. Enthält eine (relationale) Produktdatenbank auf Basis MySQL.
|


| <<Sales_Contracts_DB, Sales & Contracts Database>>
| Speichert Verkaufs- und Vertragsdaten.
| Daten liegen in einer relationalen Datenbank. Arbeitet zusammen mit dem <<Optical-Archive>>,
  sowie der <<PK-GK-Datenbank>>.
...
http://www.eyedb.org/[EyeDB^] zur Speicherung von Daten.
